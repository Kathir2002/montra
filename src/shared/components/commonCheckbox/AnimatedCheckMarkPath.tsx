...
const AnimatedCheckMarkPath: FC<Props> = memo(props => {
  const {progress, checkMarkColor} = props;
  const [length, setLength] = useState(0);
  const pathRef = useRef<React.ElementRef<typeof Path>>(null);
  ...
  return (
    <AnimatedPath
      ...
      onLayout={() => setLength(Number(pathRef?.current?.getTotalLength()))}
      ref={pathRef}
